...
현재 단계는 Analysis (Revision)이다. 본 문서는 구현 클래스를 정의하지 않고, 변경된 요구사항에서 식별되는 도메인 개념과 관계를 표현한다. 시스템 바운더리는 legacy 프로젝트와 동일하게 `rvc-controller`이며, 실제 센서 하드웨어와 actuator는 `rvc-controller` 외부 actor로 둔다.
...
| 흡입 상태 | OFF, NORMAL, POWER_UP으로 구분되는 진공 흡입 상태. 직진 주행 중에만 NORMAL 또는 POWER_UP이 될 수 있다. | RVC-FR-017 ~ RVC-FR-029, RVC-FR-031, RVC-FR-038 |
| 전방 센서 하드웨어 | 전방 장애물 interrupt 및 우측 경로 확인 시 전방 장애물 값을 제공하는 외부 actor | RVC-FR-004, RVC-FR-006, RVC-FR-009, RVC-FR-036, RVC-FR-037 |
| 측면 센서 하드웨어 | legacy 외부 actor를 유지하되 좌측 장애물 polling 입력만 제공한다. | RVC-FR-005, RVC-FR-007, RVC-FR-012, RVC-FR-013 |
| 먼지 센서 하드웨어 | 먼지 감지 값을 1 tick마다 polling 방식으로 제공하는 외부 actor | RVC-FR-024 ~ RVC-FR-029, RVC-FR-039 |
| Movement Motor | 이동 명령을 수신하는 외부 actuator | RVC-FR-030 |
| Cleaning Motor | 흡입 명령을 수신하는 외부 actuator | RVC-FR-031 |
| 우측 장애물 | 오른쪽에 존재하는 장애물이다. 직접적인 우측 센서 입력으로 감지하지 않는다. | RVC-FR-010, RVC-FR-035 |
| 우측 경로 확인 | 90도 우회전 완료 후 전방 센싱으로 기존 우측 방향의 장애물 존재 여부를 확인하는 도메인 행위 | RVC-FR-009, RVC-FR-013, RVC-FR-036, RVC-FR-037 |
| 삼방향 장애물 상황 | 전방, 좌측, 우측 장애물이 모두 존재하는 legacy 상황을 우측 경로 확인 결과로 대체 판단한 상황 | RVC-FR-011, RVC-FR-037 |
| tick | RVC controller의 1회 주기 처리 단위 | RVC-CON-008, RVC-CON-009 |
| 회전 완료 시간 | 회전 모드 진입 후 회전 완료로 간주하기까지의 시간 | RVC-FR-015, RVC-CON-003 |
...
| DR-003 | 전방 장애물이 없는 경우 좌측 장애물 입력은 회피 판단에 영향을 주지 않는다. | RVC-FR-005 |
| DR-004 | 좌회전은 기본 우선 회전 방향이다. | RVC-FR-008, RVC-FR-012 |
| DR-005 | 회전 상태는 4초 후 완료된 것으로 간주된다. | RVC-FR-015, RVC-CON-003 |
| DR-006 | 정지, 회전, 후진 중 흡입 상태는 OFF이다. | RVC-FR-017, RVC-CON-010 |
...
| DR-010 | 직접적인 우측 센서 입력은 controller 판단 입력으로 사용하지 않는다. | RVC-FR-035 |
| DR-011 | 전방 및 좌측 장애물이 감지되면 90도 우회전 완료 후 전방 센싱으로 우측 경로를 확인한다. | RVC-FR-009 |
| DR-012 | 우측 경로 확인 결과가 막혀 있으면 삼방향 장애물 상황으로 판단하고 원래 방향 복귀 후 1 tick 후진한다. | RVC-FR-011 |
| DR-013 | 3방향 장애물 처리 중 우측 경로 확인 전에 좌측 센서 polling을 1 tick마다 수행한다. | RVC-FR-013, RVC-CON-009 |
| DR-014 | simulator의 우측 장애물 정보는 분석 모델의 controller 입력 actor로 추가하지 않으며, 검증 관찰 데이터로만 보존할 수 있다. | RVC-CON-007 |
| DR-015 | movement motor에 정지 명령을 제공할 때 cleaning motor에도 OFF 명령을 제공한다. | RVC-FR-038 |
| DR-016 | 직진 주행을 시작할 때 cleaning motor의 최소 상태는 NORMAL이다. | RVC-FR-024 |
| DR-017 | 먼지 센서 하드웨어는 먼지 감지 값을 1 tick마다 polling 방식으로 제공한다. | RVC-FR-039, RVC-CON-011 |
| DR-018 | 정지, 회전, 후진 중 먼지 감지 값은 이후 POWER_UP 진입 조건으로 저장하지 않는다. 직진 주행을 시작하는 tick의 dust polling 결과로 흡입 상태를 판단한다. | RVC-FR-018, RVC-FR-019 |
